export const navItems = [
  {
    name: 'Istorijos',
    href: '/istorijos',
  },
  {
    name: 'Fotosesijos',
    href: '/fotosesijos',
  },
  {
    name: 'Apie mane',
    href: '/apie',
  },
  {
    name: 'Kontaktai',
    href: '/kontaktai',
  },
];

export const photoshootList = [
  {
    id: 'gq56djg0',
    name: 'Vaiva & Mikis',
    slug: 'vaiva-mikis',
    description: 'Vaivos ir Mikio šunybės',
    picture:
      'https://drive.google.com/uc?export=download&id=1yeca-ww_4SP7FnHSqUn5RhdCBwVa4Sw0',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1yeca-ww_4SP7FnHSqUn5RhdCBwVa4Sw0',
        alt: '',
        width: 5,
        height: 6,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1k0MF7al2qx1BItjS6c5xsctHNNUNRn5S',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1ykBxlErxrN0yR3BRxOMveb6DzmIRSH44',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1g-GkC4X28jWEMcfKn2ehmfQy5u5Jjn7K',
        alt: '',
        width: 3,
        height: 4,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1Ba2qCEwO5DC8BbKR1jI0dH5BCofr6WAY',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1iOU_HN07ul0CSIJBVY4LAirasItSUCfg',
        alt: '',
        width: 3,
        height: 4,
      },
      // {
      //   src: 'https://drive.google.com/uc?export=download&id=1ubBozN5PCzMdEAOPibdKD6-Ab0s_RxfI',
      //   alt: '',
      //   width: 2,
      //   height: 3,
      // },
      // {
      //   src: 'https://drive.google.com/uc?export=download&id=1PJG8wrYpLUlkwqqn3jiGodpkpLCKtta_',
      //   alt: '',
      //   width: 2,
      //   height: 3,
      // },
    ],
  },
  {
    id: 'g8o6djg3',
    name: 'Violeta',
    slug: 'violeta',
    description: 'Šaltis ir apsnigtas miškas',
    picture:
      'https://drive.google.com/uc?export=download&id=1FUBjm66FT_pRA5UYkbe_mxDcjLbYSf_5',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1DJtVeq_UHTgXfIhHGdRv_WSzH1PGGgdz',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1FfokPde0FySZIDYHm__ExQMjq39Qpq-j',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1a20qWZeUfSKUd-CBV3tUe1GpJ0ezWtoJ',
        alt: '',
        width: 3,
        height: 4,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1ScQGs7CsspM409v46bhYeeMVGL2EuKWT',
        alt: '',
        width: 2,
        height: 3,
      },
    ],
  },
  {
    id: 'mmd4n2bl',
    name: 'Karolina & Mantas',
    slug: 'karolina-mantas-rotonda',
    description: 'Žieminis pasivaiksčiojimas ir meškučių medžioklė',
    picture:
      'https://drive.google.com/uc?export=download&id=1dyTME0m3jl7O59gL_jMER0-9a1fw9k53',
    types: ['couple'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1GRDEYPnS5gGhzPbDCtD5PIppG0_lUNxg',
        alt: 'couple walking from side to side in park during winter',
        width: 3,
        height: 4,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1oE2SghR490Bz-heElZFIbWkeb9uLH2pv',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1gDENd8r7y1jDjXPWBfI5tV1UyekPMV1b',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1lhZKtEiP2skrYaHUKzzbJeQcteLkQbBo',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1nAmOWfv1QNpt1IoBudXaAuHh7SrixaSd',
        alt: 'a lady in warm and cozy wear sitting next to many teddy bears',
        width: 5,
        height: 6,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1G_XgYxzOhEyJJXNtEOmguTbvV8L0DsnY',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=17Zw943etimhshIks5IyywvEYiFwno-Zc',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1AtSuKNMsoNJWlX4LUMhjVAuGM7QkfjDJ',
        alt: 'man smiling and sitting next to many teddy bears',
        width: 5,
        height: 6,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1mghvVLebUJPFZ1il2J1sDHyyfQ3CnB5d',
        alt: '',
        width: 4,
        height: 5,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1bJ7T52M6euYn4lglMqwnC4AjSaBcQXDc',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1T0JcpvacQn1_AgxDBPIghCn0Tz396WQj',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1XYjAXXIZqMx8lE0a0hPytObmNpoW_7vI',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1MpfxiBucqdW0B-8huX0gl_w4wS5NOrf4',
        alt: '',
        width: 3,
        height: 4,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1y4Hx-vCPxA1hGROJdJl721DYDnmp1Oq4',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1uQIOfsZXnUds-wdFo4ObXCc2wh58ERwk',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1Xd2ACA25ABLoMbtjBEFpVKlFhoS2M6Uk',
        alt: '',
        width: 5,
        height: 7,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=14NAINkMeLfih3OLLLd7CSooMHSbS2VoJ',
        alt: '',
        width: 2,
        height: 3,
      },
      {
        src: 'https://drive.google.com/uc?export=download&id=1Sa7VhfUtjpvwaRtAMEJ1D_598KXvIu8C',
        alt: '',
        width: 2,
        height: 3,
      },
    ],
  },
  {
    id: 'gt5tdjg0',
    name: 'Loz Banditoz',
    slug: 'loz-banditoz',
    description: 'Išsivarėm motociklus žieminei fotosesijai',
    picture:
      'https://drive.google.com/uc?export=download&id=18MLmQEeFmIGGomrjjiyAOjASTWYnoo7V',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=18MLmQEeFmIGGomrjjiyAOjASTWYnoo7V',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
  {
    id: '8kadvnlh',
    name: 'Young',
    slug: 'young-kauno-eglute',
    description: 'Young ir Kauno eglutė',
    picture:
      'https://drive.google.com/uc?export=download&id=1YZZ-luI9q_NQey0H_f2NgJOTFFRM5eyL',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1YZZ-luI9q_NQey0H_f2NgJOTFFRM5eyL',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
  {
    id: 'ur7m375d',
    name: 'Ernestas',
    slug: 'linkedin-photoshoot',
    description: 'Kai reikia atsinaujint LinkedIn nuotrauką',
    picture:
      'https://drive.google.com/uc?export=download&id=18ScN1aVFDfH4UwTaSYNGqXaoBacqHczv',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=18ScN1aVFDfH4UwTaSYNGqXaoBacqHczv',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
  {
    id: 'fp45cjfz',
    name: 'Booknieta',
    slug: 'booknieta-photoshoot',
    description: 'Dominika ir jos knygų karalystė',
    picture:
      'https://drive.google.com/uc?export=download&id=1uCi7KrsGRFs4Q_OKy9ptT_zamU33n4CI',
    types: ['personal'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1uCi7KrsGRFs4Q_OKy9ptT_zamU33n4CI',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
  {
    id: 'an1ea3pv',
    name: 'Kvapni Akimirka',
    slug: 'kvapni-akimirka',
    description:
      'Kvapni akimirka - nišiniai kvepalai jaukiuose automaizeriuose',
    picture:
      'https://drive.google.com/uc?export=download&id=1zFA1kTSxuJdJzgyJHiQV0FVcMXtE7NDM',
    types: ['product'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1zFA1kTSxuJdJzgyJHiQV0FVcMXtE7NDM',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
  {
    id: 'us8n486e',
    name: 'Glorija',
    slug: 'glorijai-1',
    description: 'Glorijos 1-asis tortadienis',
    picture:
      'https://drive.google.com/uc?export=download&id=1V72m07BxFnO38pj5YJW1UnAsAHadPoMj',
    types: ['family'],
    pictures: [
      {
        src: 'https://drive.google.com/uc?export=download&id=1V72m07BxFnO38pj5YJW1UnAsAHadPoMj',
        alt: '',
        width: 5,
        height: 6,
      },
    ],
  },
];
